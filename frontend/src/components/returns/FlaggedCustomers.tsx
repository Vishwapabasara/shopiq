  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-5 py-3 border-b border-zinc-100 flex items-center justify-between">
        ...
        <p className="text-xs text-zinc-400">{customers.length} flagged</p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-zinc-50">
            {customers.map(c => (
              <tr key={c.customer_id} className="hover:bg-zinc-50/60 transition-colors">
                <td className="px-5 py-3">
                  <p className="font-medium text-zinc-800">{c.name || 'Guest'}</p>
                  <p className="text-xs text-zinc-400">{c.email}</p>
                </td>
                <td className="text-right px-4 py-3 text-zinc-600">{c.total_orders}</td>
                <td className="text-right px-4 py-3 text-zinc-600">{c.total_returns}</td>
