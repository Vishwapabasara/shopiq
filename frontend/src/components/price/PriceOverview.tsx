    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card p-4">
          <p className="text-xs text-zinc-500 font-medium uppercase tracking-wide mb-1">Products Checked</p>
          <p className="text-2xl font-semibold tabular-nums text-zinc-800">{results.total_products}</p>
          <p className="text-xs text-zinc-400 mt-0.5">Against live market prices</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-zinc-500 font-medium uppercase tracking-wide mb-1">Overpriced</p>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">Significantly above market</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-zinc-500 font-medium uppercase tracking-wide mb-1">Being Undercut</p>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">Competitors slightly cheaper</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-zinc-500 font-medium uppercase tracking-wide mb-1">Competitive</p>
          <p className="text-2xl font-semibold tabular-nums text-emerald-600">{results.products_competitive}</p>
          <p className="text-xs text-zinc-400 mt-0.5">Well priced vs market</p>
        </div>
      </div>
      ...
      <div className="card px-5 py-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-semibold text-zinc-800">Market Position Distribution</p>
          <p className="text-xs text-zinc-400">{checked} products with competitor data</p>
        </div>
        <div className="flex h-3 rounded-full overflow-hidden gap-0.5 mb-3">
          ...
          {results.products_no_data > 0 && (
            <div
              className="bg-zinc-200 transition-all duration-700"
        ...
        <div className="flex flex-wrap gap-4 text-xs text-zinc-500">
          ...
          {results.products_no_data > 0 && (
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-zinc-300" />{results.products_no_data} no data</span>
          )}
        </div>
      </div>
      ...
      {results.insights.length > 0 && (
        <div className="card px-5 py-4">
          <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-3">Key Insights</p>
      ...
      {results.top_competitors.length > 0 && (
        <div className="card px-5 py-4">
          <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-3">Top Competitors Across Your Catalogue</p>
          <div className="flex flex-wrap gap-2">
            {results.top_competitors.map(c => (
              <div key={c.name} className="flex items-center gap-1.5 bg-zinc-50 border border-zinc-200 rounded-lg px-3 py-1.5">
                <span className="text-sm font-medium text-zinc-700">{c.name}</span>
                <span className="text-xs text-zinc-400">{c.count} product{c.count !== 1 ? 's' : ''}</span>
              </div>
            ))}
          </div>
        </div>
