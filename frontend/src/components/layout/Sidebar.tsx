...
const PLAN_BADGE: Record<string, { label: string; cls: string }> = {
  free:       { label: 'Free',       cls: 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400' },
  pro:        { label: 'Pro',        cls: 'bg-brand-100 text-brand-700 dark:bg-brand-200/10 dark:text-brand-300' },
  enterprise: { label: 'Enterprise', cls: 'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' },
  starter:    { label: 'Free',       cls: 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400' },
}
...
export function Sidebar({ mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const sidebarContent = (
    <aside className="w-56 flex-shrink-0 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 flex flex-col h-full">

      {/* Logo */}
      <div className="px-4 py-4 border-b border-zinc-100 dark:border-zinc-800">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded bg-brand-200 dark:bg-brand-200/15 flex items-center justify-center flex-shrink-0">
            <img src={logo} alt="ShopIQ" className="w-5 h-5 object-contain" />
          </div>
          <div className="flex-1 min-w-0">
            <span className="font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight text-sm">ShopIQ</span>
            <p className="text-[10px] text-zinc-400 dark:text-zinc-500 leading-tight tracking-wide uppercase font-medium">Shopify Intelligence</p>
          </div>
          <div className="flex items-center gap-1 flex-shrink-0">
            <button
              onClick={toggle}
              className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800"
            ...
            <button
              ...
              className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors hidden lg:flex p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800"
            ...
            {onMobileClose && (
              <button
                ...
                className="lg:hidden text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors ml-1"
              >
                {Icons.close}
              </button>
            )}
      ...
      <div className="px-3 py-2.5 border-b border-zinc-100 dark:border-zinc-800">
        <div className="flex items-center gap-2 bg-zinc-50 dark:bg-zinc-800/60 rounded px-3 py-2">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0" />
          <span className="text-xs text-zinc-600 dark:text-zinc-300 truncate font-medium flex-1 min-w-0">
      ...
      {/* Usage quick-view (free plan) */}
      {(plan === 'free' || plan === 'starter') && usageData && (
        <div className="px-3 pb-2">
          <div className="bg-zinc-50 dark:bg-zinc-800/60 rounded px-3 py-2.5 space-y-1.5">
            <p className="text-[10px] font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-1">This month</p>
      ...
      <div className="border-t border-zinc-100 dark:border-zinc-800 px-3 py-3">
        <div className="flex items-center gap-2.5">
          <NavLink
            ...
            onClick={onMobileClose}
            className="flex items-center gap-2.5 flex-1 min-w-0 rounded hover:bg-zinc-50 dark:hover:bg-zinc-800 px-1.5 py-1 -mx-1.5 transition-colors"
          >
            <div className="w-7 h-7 rounded-full bg-brand-100 dark:bg-brand-200/15 flex items-center justify-center flex-shrink-0">
              <span className="text-brand-700 dark:text-brand-300 text-[11px] font-semibold">{initials}</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-zinc-700 dark:text-zinc-300 truncate">
                {me?.shop_name ?? me?.shop_domain}
              </p>
              <p className="text-[11px] text-zinc-400 dark:text-zinc-500 capitalize">{badge.label} plan</p>
            </div>
          </NavLink>
          <button
            onClick={() => authApi.logout().then(() => navigate('/login'))}
            className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors flex-shrink-0 p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800"
            title="Sign out"
          >
            {Icons.logout}
          </button>
...
function QuickUsageRow({ label, used, limit }: { label: string; used: number; limit: number }) {
  ...
  const color = pct >= 90 ? 'bg-red-400' : pct >= 70 ? 'bg-amber-400' : 'bg-brand-300'
  return (
    <div>
      <div className="flex justify-between text-[10px] text-zinc-400 dark:text-zinc-500 mb-0.5">
        <span>{label}</span>
        <span className="font-mono">{used}/{limit === -1 ? '∞' : limit}</span>
      </div>
...
function NavSection({ label, items, onNavClick }: { label: string; items: NavItem[]; onNavClick?: () => void }) {
  return (
    <div>
      <p className="text-[10px] font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest px-2 mb-1.5">{label}</p>
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) => cn(
        ...
        isActive
          ? 'bg-brand-50 dark:bg-brand-200/8 text-brand-700 dark:text-brand-300 font-medium'
...
function ComingSoonItem({ item }: { item: NavItem }) {
  return (
    <div className="flex items-center gap-2.5 px-2.5 py-2 rounded text-sm text-zinc-300 dark:text-zinc-600 cursor-default">
      <span className="flex-shrink-0 w-4 flex items-center justify-center">{item.icon}</span>
      <span>{item.label}</span>
      <span className="ml-auto text-[9px] font-medium bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-500 px-1.5 py-0.5 rounded border border-amber-100 dark:border-amber-800">Soon</span>
    </div>
  )
}
